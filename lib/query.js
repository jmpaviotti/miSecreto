function selectQuery(req, page, expressions) {
  ...
  if (req.term) {
    query = {
      text: `SELECT * FROM secretos WHERE content ILIKE ALL($1) ORDER BY ${sortby} ${order} LIMIT 10 OFFSET ${offset}`,
      values: [expressions],
    };
  } else {
...
function countQuery(terms) {
  ...
  if (!terms) {
    ...
    query = {
      text: "SELECT COUNT(*) FROM secretos WHERE content ILIKE ALL($1)",
      values: [terms],
    };
  }
